...
# %% auto 0
__all__ = ['base_path', 'get_request_headers', 'scrape_nse_holidays', 'get_holidays_table', 'save_daily_bhavcopy',
           'fetch_bhavcopy_data_for_range', 'scrape_symbol_list', 'scrape_asm_list', 'scrape_gsm_list', 'get_blocklist']
...
def scrape_nse_holidays(s, segment):
    ...
    r = s.get("https://www.nseindia.com/", headers=headers, timeout=5)
    cookies = dict(r.cookies)
    r = s.get(holidays_url, headers=headers, timeout=5, cookies=cookies)
...
def get_holidays_table(segment="Equities"):
    holidays_path = Path(
    ...
    if not holidays_path.exists():
        ...
        with requests.Session() as s:
            df = scrape_nse_holidays(s, segment)
            df.to_csv(holidays_path, index=False)
...
def scrape_symbol_list(file_url, file_path=None):
    headers = get_request_headers()
    print(f"Downloading:{file_url}")
